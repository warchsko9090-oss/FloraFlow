...
def seed_knowledge():
    """Добавляет примеры SQL для чата, если их ещё нет. Возвращает число добавленных."""
    examples = [
        # === 1. ЗАКАЗЫ ===
    ...
    existing = {
        (q or '').strip().lower()
        for (q,) in db.session.query(SQLExample.question).all()
    }
    added = 0
    for ex in examples:
        key = (ex.get('q') or '').strip().lower()

        ...
        for year, rows in data_by_year.items():
            ...
            for r in rows:
                ...
                if pid and sid and fid:
                    ...
                    # Защита: импорт не должен опускать остаток ниже уже
                    # зарезервированного по активным заказам — иначе свободно
                    # станет отрицательным, а отгрузки уведут склад в минус.
                    reserved = compute_reserved(pid, sid, fid, year)
                    if new_qty < reserved:
                        raise ValueError(
                            f'Импорт нельзя применить: по партии '
                            f'(растение #{pid}, размер #{sid}, поле #{fid}, '
                            f'год {year}) уже зарезервировано {reserved} шт, '
                            f'а в файле {new_qty} шт. Сначала отмените или '
                            f'скорректируйте заказы.'
                        )

                    st = get_or_create_stock(pid, sid, fid, year)
                    
                    # Вычисляем дельту для истории, чтобы отчёты не двоились.

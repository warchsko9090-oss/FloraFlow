...
def _table_columns(table_name: str) -> dict:
    """Имя столбца -> метаданные (SQLite и PostgreSQL)."""
...
def ensure_digging_table_exists():
    ...
    existing_cols = _table_columns('digging_log')

    # Только SQLite: в старой схеме order_item_id мог быть NOT NULL
    if _is_sqlite_engine():
        rows = db.session.execute(text("PRAGMA table_info('digging_log')")).fetchall()
        sqlite_cols = {row[1]: row for row in rows}
        if 'order_item_id' in sqlite_cols and sqlite_cols['order_item_id'][3] == 1:
...
def ensure_payment_file_column():
    ...
    if not inspect(db.engine).has_table('payment'):
        return
    existing_cols = _table_columns('payment')
...
def mobile_index():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            tg_order_ids = set()
            # Список созданных DiggingLog — нужен для хука apply_planned_consume_for_log
            new_logs = []
            ...
            if count_saved > 0:
                ...
                # Хук: для каждой новой выкопки создаём плановое списание
                # ВиУМ по тех.карте (plant, size). Если карты нет — просто
                # пропускаем; выкопка не должна падать из-за ВиУМ.
                try:
                    from app import vium_techcard
                    planned_total = 0
                    for nlog in new_logs:
                        planned_total += vium_techcard.apply_planned_consume_for_log(nlog)
                    if planned_total:
                        db.session.commit()
                except Exception:
                    db.session.rollback()
                    try:
                        current_app.logger.exception(
                            'vium_techcard.apply_planned_consume_for_log failed'
                        )
                    except Exception:
                        pass

                # Триггер: «начали копать заказ без счёта». Шлём один раз
                # на заказ (dedup_key=order). Менеджеру (по created_by_user_id)
                # и админам/исполнителям (по роли).
                try:
                    from app.anomaly_engine import check_digging_started_without_invoice
                    for oid in tg_order_ids:
                        check_digging_started_without_invoice(oid)
                except Exception:
                    try:
                        current_app.logger.exception(
                            'check_digging_started_without_invoice failed'
                        )
                    except Exception:
                        pass
...
def manager_report():
    ...
    if request.method == 'POST':
        try:
            ...
            # 0) Добавление строки админом (без отправки в Telegram)
            if action == 'add_log_admin':
                if current_user.role != 'admin':
                    flash('Доступ запрещен', 'danger')
                    return redirect(url_for('digging.manager_report', date=target_date))

                try:
                    add_item_id = int(request.form.get('add_order_item_id') or 0)
                    add_qty = int(request.form.get('add_quantity') or 0)
                except (TypeError, ValueError):
                    flash('Проверьте позицию и количество.', 'warning')
                    return redirect(url_for('digging.manager_report', date=target_date))

                if add_item_id <= 0 or add_qty <= 0:
                    flash('Количество должно быть больше 0.', 'warning')
                    return redirect(url_for('digging.manager_report', date=target_date))

                item = OrderItem.query.get(add_item_id)
                if not item or not item.order or item.order.is_deleted:
                    flash('Позиция заказа не найдена.', 'warning')
                    return redirect(url_for('digging.manager_report', date=target_date))

                max_left = (item.quantity or 0) - (item.dug_total or 0)
                if add_qty > max_left:
                    flash(
                        f'Нельзя добавить {add_qty} шт. (осталось {max_left} шт. по позиции).',
                        'danger'
                    )
                    return redirect(url_for('digging.manager_report', date=target_date))
                ...
                db.session.flush()
                recalc_affected(item.plant_id, item.size_id, item.field_id, item.year, item.id)
                try:
                    from app import vium_techcard
                    vium_techcard.apply_planned_consume_for_log(add_log)
                except Exception:
                    current_app.logger.exception('vium_techcard.apply_planned_consume_for_log (admin add) failed')
                db.session.commit()
                flash('Строка добавлена администратором.', 'success')
                log_action(
                    f"Админ добавил строку выкопки: заказ #{item.order_id}, "
                    f"{item.plant.name if item.plant else 'позиция'} {add_qty} шт. за {target_date}"
                )
                return redirect(url_for('digging.manager_report', date=target_date))
            ...
            for key in request.form.keys():
                ...
                if log_entry and (log_entry.status == 'pending' or current_user.role == 'admin'):
                    ...
                    # Снимаем плановое списание ВиУМ ДО удаления лога,
                    # чтобы не потерять каскад по FK.
                    try:
                        from app import vium_techcard
                        vium_techcard.delete_planned_for_log(log_entry.id)
                    except Exception:
                        current_app.logger.exception('vium_techcard.delete_planned_for_log failed')
            ...
            for key, val in request.form.items():
                ...
                if log_entry and (log_entry.status == 'pending' or current_user.role == 'admin') and log_entry.quantity != new_qty:
                    ...
                    # Перерасчёт плановых ВиУМ-расходов под новое qty
                    try:
                        from app import vium_techcard
                        vium_techcard.replace_planned_for_log(log_entry)
                    except Exception:
                        current_app.logger.exception('vium_techcard.replace_planned_for_log failed')
                    changed_count += 1

            # 3) Привязка незакрепленных записей к заказам (распределение)
            for key, val in request.form.items():
    ...
    if current_user.role == 'admin':
        ...
        for it in open_items:
            ...
            admin_add_items.append({
                ...
                'client_name': it.order.client.name if it.order and it.order.client else '—',
    ...
    return render_template(
        'digging/digging_report.html',
        logs=logs,
        target_date=target_date,
        candidates=candidates,
        admin_add_items=admin_add_items
    )
...
def analytics():
    ...
    # 1) Кол-во сотрудников, работавших в день (часы > 1 по любой ставке) — см. timesheet_worker_sets_by_date
    from app.utils import timesheet_worker_sets_by_date, timelog_date_str
    worked = timesheet_worker_sets_by_date(start_date, end_date)
    ...
    workers_per_day = [len(worked.get(timelog_date_str(d), set())) for d in days]
...
def digging_planning():
    ...
    # 2. ГЕНЕРИРУЕМ СКРОЛЛ-ЛЕНТУ КАЛЕНДАРЯ ПО НЕДЕЛЯМ
    # Центральная неделя — параметр ?center=YYYY-MM-DD (понедельник). Если
    # параметра нет — берём текущую неделю. Лента из 17 недель (8 до + центр
    # + 8 после) — этого с запасом хватает на видимые ±2 недели и ещё на
    # прокрутку без релоадов.
    today = msk_today()
    weeks_before = 8
    weeks_after = 8
    ...
    if center_str:
        try:
            d = datetime.strptime(center_str, '%Y-%m-%d').date()
            center_monday = d - timedelta(days=d.weekday())
        except Exception:
            center_monday = None
    ...
    resp = make_response(render_template('digging/planning.html',
                           ...
                           center_monday_iso=center_monday.strftime('%Y-%m-%d'),

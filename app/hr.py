...
def _push_day_off_with_hours_anomaly(log):
    """Создаёт/обновляет TgTask для admin/executive: бригадир/user2 оформил
    выходной с часами. ЗП будет начислена 10ч + фактические часы дня — это
    нормальный рабочий сценарий, но требует подтверждения руководителя.
    """
    try:
        emp = log.employee
    except Exception:
        emp = None
    emp_name = (emp.name if emp else f'#{log.employee_id}')
    h_total = float(
        (log.hours_norm or 0) + (log.hours_norm_over or 0)
        + (log.hours_spec or 0) + (log.hours_spec_over or 0)
    )
    dedup = f'hr_dayoff_with_hours:{log.employee_id}:{log.date.isoformat()}'
    title = f'Выходной с часами: {emp_name} — {log.date.strftime("%d.%m.%Y")} ({_format_hours_value(h_total)}ч)'
    details = (
        f'Бригадир/user2 оформил день как «выходной» и одновременно проставил часы.\n'
        f'ЗП за этот день = 10ч (оплачиваемый выходной) + {_format_hours_value(h_total)}ч '
        f'по соответствующим ставкам. Проверьте, что это корректно.'
    )

    task = TgTask.query.filter_by(dedup_key=dedup).first()
    now = msk_now()
    if task is None:
        task = TgTask(
            raw_text=title,
...
def personnel():
    ...
    if request.method == 'POST':
        ...
        if action == 'save_settings' and current_user.role == 'admin':
            ...
            # Часы пишем как есть всегда: при выходном с часами они оплачиваются
            # СВЕРХ 10ч (paid_day_off) — админ дополнительно увидит триггер.
            ...
            # Сигнал админам: бригадир/user2 оформил выходной с фактическими часами.
            try:
                if (
                    is_day_off
                    and current_user.role in ('user2', 'brigadier')
                    and (log.hours_norm + log.hours_norm_over + log.hours_spec + log.hours_spec_over) > 0
                ):
                    _push_day_off_with_hours_anomaly(log)
            except Exception:
                current_app.logger.exception('add_hours: anomaly task failed')
    ...
    for l in all_logs:
        ...
        # День-выходной попадает в day_offs (для +10ч × norm). Если на этом
        # же дне есть фактические часы — они платятся СВЕРХ 10ч по обычным
        # ставкам (norm/over/spec/...), как и в обычный рабочий день.
        if getattr(l, 'is_day_off', False):
        ...
        if not getattr(l, 'is_day_off', False) and (hn + hno + hs + hso) > 1:

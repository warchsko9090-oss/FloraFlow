...
def _card_topic(card):
    ...
    if t == 'anomaly':
        ...
        if kind.startswith('stale_') or kind.startswith('debtor'):
            return 'sales'
        if kind.startswith('year_'):
            # year_no_batch / year_overcommit / year_no_stock_anywhere
            return 'sales'
        ...
        if kind.startswith('stock_') or kind.startswith('digging_'):
            # digging_no_invoice — это про продажу/счёт, а не про поле.
            if kind == 'digging_no_invoice':
                return 'sales'
            # stock_no_price — про справочник цен, который ведёт админ.
            # В тематической вкладке «Финансы» (там админ работает с ценами
            # и расходами) карточка релевантнее, чем в «Поле».
            if kind == 'stock_no_price':
                return 'finance'
            return 'field'
        if kind.startswith('recount_'):
            return 'field'
        return 'other'
    if t in ('tg_task',):
        ...
        if at in _TG_TASK_FIELD_TYPES:
            return 'field'
        if at in ('create_order', 'shipment', 'order', 'paid_order_change'):
            return 'sales'
...
def _collect_kpis(user, today):
    ...
    # Только админ/исполнительный директор. Бригадиру и менеджеру эта плитка
    # не нужна — у бригадира своя страница с задачами, а менеджер копкой не
    # занимается.
    if is_boss:
        try:
            ...
            for d in drafts:
                try:
                    card = {
                        ...
                        'text': 'Требует согласования менеджером питомника и менеджером сайта.',
                        'url': url_for('orders.client_draft_detail', doc_id=d.id),
                        'date_str': (d.date or today).strftime('%d.%m.%Y'),
                        'raw_date': today,
                    }
                    add_to_group(card, today)
                except Exception:
                    current_app.logger.exception('feed: draft card failed')
        except Exception:
            current_app.logger.exception('feed: drafts block failed')

    # 2b. Заявки «По запросу» с витрины
    if current_user.role in ['admin', 'user', 'executive']:
        try:
            from app.models import ShopOnRequest
            pending = ShopOnRequest.query.filter_by(status=ShopOnRequest.STATUS_NEW).order_by(
                ShopOnRequest.created_at.desc()
            ).limit(20).all()
            for r in pending:
                try:
                    plant_name = r.plant.name if r.plant else 'Главная страница'
                    size_name = r.size.name if r.size else 'Прайс / консультация'
                    card = {
                        'id': f'shop_on_request_{r.id}',
                        'type': 'shop_on_request',
                        'title': f'По запросу: {plant_name}' if r.plant else 'Заявка с главной: прайс',
                        'text': f'{r.customer_name} · {r.phone or size_name}',
                        'url': url_for('orders.shop_on_request_detail', req_id=r.id),
                        'date_str': (r.created_at.date() if r.created_at else today).strftime('%d.%m.%Y'),
                        'raw_date': r.created_at.date() if r.created_at else today,
                    }
                    add_to_group(card, today)
                except Exception:
                    current_app.logger.exception('feed: shop on-request card failed')
        except Exception:
            current_app.logger.exception('feed: shop on-request block failed')

    # 3. Выкопка (Задачи) — работа бригады. В дайджесте показываем только
    # админу (в scope='all' и scope='field'). Менеджер (user) копку не видит
    # никогда — это не его участок работ. Бригадир (user2) видит свою копку
    # на отдельных страницах (/digging, мобильный ввод и т.д.) — в дайджесте
    # она ему не нужна, чтобы не забивала ленту.
    _show_digging = (
        is_admin and admin_scope in ('all', 'field')
    )
    if _show_digging:
        try:
...
@bp.route('/dashboard/anomalies', methods=['GET', 'POST'])
@login_required
def anomaly_history():
    ...
    # Сохранение настройки порога — отдельной формой на этой же странице.
    if request.method == 'POST' and request.form.get('action') == 'save_threshold':
        if (current_user.role or '') != 'admin':
            flash('Менять порог аномалии может только админ.', 'warning')
            return redirect(url_for('main.anomaly_history'))
        try:
            v = set_anomaly_threshold_pct(request.form.get('threshold_pct'))
            flash(f'Порог аномалии сохранён: {v:.1f} %', 'success')
    ...
    return render_template(
        ...
        digests=digests,
        today=msk_today(),
        anomaly_threshold_pct=get_anomaly_threshold_pct(),
    )
...
def complete_task():
    ...
    if task and task.status == 'pending' and fact_qty > 0:
        ...
        db.session.commit()
        try:
            from app import vium_techcard
            vium_techcard.apply_planned_consume_for_log(log)
            db.session.commit()
        except Exception:
            db.session.rollback()
            from flask import current_app
            current_app.logger.exception('vium_techcard.apply_planned_consume_for_log (complete_task) failed')
...
def favicon_legacy():
    """Legacy favicon path used by search crawlers."""
    try:
        return send_file('static/icon-192.png', mimetype='image/png')
    except (OSError, FileNotFoundError):
        try:
            return send_file('static/img/favicon-knyajestvo.svg', mimetype='image/svg+xml')
        except (OSError, FileNotFoundError):
            return "", 404
...
def inject_global_vars():
    ...
    try:
        if current_user.is_authenticated:
            ...
            # --- 4) Заявки с витрины (КП + «По запросу» + форма главной) ---
            if role in ('admin', 'user', 'executive'):
                from app.shop_on_request import count_dashboard_site_alerts
                counts['today'] += count_dashboard_site_alerts(role)
    except Exception:
...
def api_visual_move():
    ...
    try:
        ...
        # Учитываем резерв: нельзя унести больше, чем свободно (даже если
        # физически на поле столько есть — часть может быть забронирована).
        from app.stock_helpers import compute_reserved
        reserved_src = compute_reserved(int(data['plant_id']), int(data['size_id']),
                                        int(data['field_from']), int(data['year']))
        free_src = max(0, int(src_stock.quantity or 0) - reserved_src)
        if qty > free_src:
            return jsonify({
                'status': 'error',
                'message': f'На исходном поле свободно {free_src} шт '
                           f'(всего {src_stock.quantity}, в резерве {reserved_src}).'
            })

        src_stock.quantity -= qty
        dest_stock.quantity += qty
        # Переносим цены партии в новое поле, если там пусто.
        try:
            if (not dest_stock.purchase_price) and src_stock.purchase_price:
                dest_stock.purchase_price = src_stock.purchase_price
            if (not dest_stock.price) and src_stock.price:
                dest_stock.price = src_stock.price
        except Exception:
            pass
...
def api_visual_income():
    ...
    try:
        ...
        st = get_or_create_stock(plant_id, size_id, field_to, year)
        st.quantity += qty
        try:
            in_purchase = float(data.get('purchase_price') or 0)
        except (ValueError, TypeError):
            in_purchase = 0
        try:
            in_sell = float(data.get('price') or 0)
        except (ValueError, TypeError):
            in_sell = 0
        try:
        ...
        apply_purchase_lot(
            plant_id=plant_id, size_id=size_id, field_id=field_to, year=year,
            quantity=qty, purchase_price=in_purchase, supplier_id=supplier_id,
...
def api_visual_writeoff():
    ...
    if current_user.role not in ('admin', 'user'):
        return jsonify({'status': 'error', 'message': 'Недостаточно прав для списания'})
    data = request.json or {}
    try:
        qty = int(data.get('qty', 0))
        year = int(data['year'])
...
def api_visual_field_recount():
    ...
    try:
        ...
        # Сбой здесь не должен ломать ответ пользователю.
        try:
            ...
            try:
                current_app.logger.exception(
                    'sync_recount_anomaly_for_doc(%s) failed (visual)', doc.id
                )
            except Exception:
                pass

...
def order_detail(order_id):
    ...
    if request.method == 'POST':
        ...
        if 'update_info' in request.form:
            ...
            p = Payment.query.get(request.form.get('payment_id'))
            if p:
                if (p.payment_type or '') == 'writeoff' and current_user.role != 'admin':
                    flash('Удаление списания долга доступно только администратору', 'danger')
                    return redirect(url_for('orders.order_detail', order_id=o.id))
                amount = p.amount
                ptype = p.payment_type or 'cashless'
                db.session.delete(p)
                db.session.commit()
                if ptype == 'writeoff':
                    flash('Списание долга удалено')
                    log_action(f"Удалил списание долга в заказе #{o.id}: {amount}")
                else:
